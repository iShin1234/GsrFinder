 ...
 */
package sg.edu.smu.gsrfinder.common.rendering
...
class BackgroundRenderer {
    ...
    fun createOnGlThread(context: Context?, depthTextureId: Int =  /*depthTextureId=*/-1) {
        ...
        run {
            ...
            val fragmentShader = context?.let {
                ShaderUtil.loadGLShader(
                    TAG, it, GLES20.GL_FRAGMENT_SHADER, CAMERA_FRAGMENT_SHADER_NAME
                )
            }
        ...
        run {
            val vertexShader = context?.let {
                ShaderUtil.loadGLShader(
                    TAG, it, GLES20.GL_VERTEX_SHADER, DEPTH_VISUALIZER_VERTEX_SHADER_NAME
                )
            }
            val fragmentShader = context?.let {
                ShaderUtil.loadGLShader(
                    TAG, it, GLES20.GL_FRAGMENT_SHADER, DEPTH_VISUALIZER_FRAGMENT_SHADER_NAME
                )
            }
